...
| `check-pre-push` | husky — parallel leaves plus `knip-warn` |
...
"dev:vite": "FORCE_COLOR=1 bun --bun vite dev --host 127.0.0.1 --strictPort",
...
Existing pipeline/analysis/MapRoulette CLIs (`pipeline:*`, `analysis:*`, `maproulette:*`, `changelog:*`) predate this rule and stay as operator-facing names; do not treat them as Bun glob groups. `update` is a real sequential group (`update:*`).
